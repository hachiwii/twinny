...
describe("WorkspaceManager", () => {
  ...
  it("continues to resolve legacy colon conversation keys without renaming them", () => {
    const manager = WorkspaceManager.fromTwinnyHome(tempDir);

    const workspace = manager.ensureWorkspace("p2p:ou_abc");

    expect(workspace).toBe(path.join(tempDir, "workspaces", "p2p:ou_abc"));
    expect(fs.statSync(workspace).isDirectory()).toBe(true);
    expect(getP2PChatIdFromConversationKey("p2p:ou_abc")).toBe("ou_abc");
    expect(getGroupChatIdFromConversationKey("group:oc_group")).toBe("oc_group");
  });
});
