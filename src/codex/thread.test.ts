...
describe("codex thread payloads", () => {
  it("builds thread/start with Twinny's thread runtime overrides", () => {
    expect(buildThreadStartParams({ cwd: "/tmp/twinny/workspaces/p2p:ou_1" })).toEqual({
  ...
  it("builds thread/resume with the persisted thread id and the same thread overrides", () => {
    expect(buildThreadResumeParams("thread_123", { cwd: "/tmp/twinny/workspaces/p2p:ou_1" })).toEqual({
      threadId: "thread_123",
      cwd: "/tmp/twinny/workspaces/p2p:ou_1",
      approvalPolicy: "never",
      persistExtendedHistory: true
